#
# Script to deploy OpenShift Data Foundation onto OpenShift
#
...
LOCATION_STRING=$(oc get nodes | grep worker |head -1 | awk '{print $1}'  | awk '{split($0,loc,"-"); print loc[4]}')
...
log-output "ODF successfully installed on cluster $API_SERVER"