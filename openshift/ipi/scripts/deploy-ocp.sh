...
OUTPUT_FILE="script-output-$(date -u +'%Y-%m-%d-%H%M%S').log"
...
########
# Set resource group if not already (assumes managed identity assigned to a single resource group)
if [[ -z $RESOURCE_GROUP ]]; then 
    resource_groups=$(az group list --query '[].name')
